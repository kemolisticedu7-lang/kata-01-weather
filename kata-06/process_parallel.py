...
def process_chunk(args: Tuple[int, List[float], bool]) -> ChunkResult:
    chunk_index, chunk, simulate_bug = args

    # Example failure handling: intentionally crash one worker if simulate_bug is True
    # (We’ll use this later for git bisect)
    if simulate_bug and chunk_index == 2:
        raise ValueError("Intentional bug: chunk 2 fails")
...
def main() -> None:
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        futures = {
            ex.submit(process_chunk, (chunk_index, chunk, simulate_bug)): chunk_index
    ...
    print(f"[INFO] Errors: {ERROR_PATH}")
